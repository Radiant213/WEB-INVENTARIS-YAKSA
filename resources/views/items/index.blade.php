...
<div x-data="masterBarang()" x-init="init()">
    ...
    <div x-show="!loaded" x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                <div class="skeleton skeleton-title" style="width: 170px;"></div>
                <div class="skeleton skeleton-text-sm" style="width: 320px;"></div>
            </div>
            <div class="skeleton" style="width: 140px; height: 40px; border-radius: 0.75rem;"></div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center mb-4 space-y-3 sm:space-y-0">
            ...
            <div class="flex gap-2">
                @for($s = 0; $s < 3; $s++)
                <div class="skeleton" style="width: {{ rand(100, 140) }}px; height: 38px; border-radius: 0.75rem;"></div>
                @endfor
        ...
        <div class="skeleton-card" style="padding:0;">
            <div class="overflow-hidden">
                ...
                <div class="skeleton-table-row" style="background:#fafafa; border-bottom:1px solid #e5e7eb;">
                    ...
                    <div class="skeleton skeleton-text-sm" style="width:100px; margin-bottom:0; flex:1;"></div>
                    <div class="skeleton skeleton-text-sm" style="width:50px; margin-bottom:0; flex-shrink:0;"></div>
                    <div class="skeleton skeleton-text-sm" style="width:50px; margin-bottom:0; flex-shrink:0;"></div>
                    <div class="skeleton skeleton-text-sm" style="width:70px; margin-bottom:0; flex-shrink:0;"></div>
                    <div class="skeleton skeleton-text-sm" style="width:50px; margin-bottom:0; flex-shrink:0;"></div>
                    <div class="skeleton skeleton-text-sm" style="width:50px; margin-bottom:0; flex-shrink:0;"></div>
                </div>
                <!-- Table Rows -->
                ...
                <div class="skeleton-table-row">
                    <div class="skeleton" style="width:1.25rem; height:1.25rem; border-radius:0.375rem; flex-shrink:0;"></div>
                    <div class="skeleton skeleton-text" style="width: {{ rand(120, 200) }}px; margin-bottom:0; flex:1;"></div>
                    <div class="skeleton skeleton-text" style="width: {{ rand(80, 120) }}px; margin-bottom:0; flex-shrink:0; font-family:monospace;"></div>
                    ...
                    <div class="flex items-center gap-2 flex-shrink-0">
                        <div class="skeleton" style="width:1.25rem; height:1.25rem; border-radius:0.375rem;"></div>
                        <div class="skeleton" style="width:1.25rem; height:1.25rem; border-radius:0.375rem;"></div>
                    </div>
                </div>
                @endfor
            ...
            <!-- Skeleton Pagination -->
            <div style="padding:1rem 1.5rem; border-top:1px solid #f3f4f6; background:#fafafa; display:flex; justify-content:space-between; align-items:center;">
                <div class="skeleton skeleton-text-sm" style="width: 120px; margin-bottom:0;"></div>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-4 space-y-3 sm:space-y-0"
        ...
        <div class="flex flex-wrap w-full sm:w-auto gap-2">
            <form method="GET" action="{{ route('items.index') }}" class="flex flex-wrap gap-2 w-full sm:w-auto items-center" id="filterForm">
                                        ...
                                        <div class="border border-gray-200 rounded-xl text-xs bg-white overflow-hidden">
                                            <table class="w-full">
                                                <tr class="bg-gray-50 text-gray-500">
                                                    ...
                                                    <th class="p-3 text-left font-semibold">Bukti</th>
                                                ...
                                                <tr class="border-t border-gray-100 hover:bg-gray-50/50 transition-colors duration-150">
                                                    ...
                                                    <td class="p-3">
                                                        @if($tx->bukti_foto)
                                                            @if(str_ends_with(strtolower($tx->bukti_foto), '.pdf'))
                                                                <a href="{{ url('file/' . $tx->bukti_foto) }}" target="_blank" class="group relative inline-flex transition-transform hover:scale-110 items-center justify-center w-8 h-8 bg-red-50 rounded-md border border-red-200 text-red-500" title="Lihat PDF">
                                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg>
                                                                    <div class="absolute inset-0 bg-black/10 opacity-0 group-hover:opacity-100 rounded-md transition-opacity flex items-center justify-center">
                                                                        <svg class="w-3 h-3 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
                                                                    </div>
                                                                </a>
                                                            @else
                                                                <button type="button" @click="Swal.fire({
                                                                    ...
                                                                    <img src="{{ url('file/' . $tx->bukti_foto) }}" class="w-8 h-8 object-cover rounded-md border border-gray-200 shadow-sm mx-auto">
                                                                    <div class="absolute inset-0 bg-black/10 opacity-0 group-hover:opacity-100 rounded-md transition-opacity flex items-center justify-center">
                                                                        <svg class="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                                                                    </div>
                                                                </button>
                                                            @endif
                                                        @else
                                                    ...
                                                    <td class="p-3 text-center">
                                                        <div class="flex items-center justify-center gap-2">
                                                            <a href="{{ route('transactions.edit', $tx->id) }}" class="inline-flex items-center justify-center text-gray-400 hover:text-yaksa-red transition-all duration-200 hover:scale-110" title="Edit Transaksi">
                                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                                            </a>
                                                            <form id="delete-tx-{{ $tx->id }}" method="POST" action="{{ route('transactions.destroy', $tx->id) }}" class="hidden">
                                                                @csrf @method('DELETE')
                                                            </form>
    ...
    <div x-show="modalOpen" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center">
        ...
        <div x-show="modalOpen"
            ...
            <form method="POST" action="{{ route('transactions.store') }}" class="p-6 space-y-6 flex-1 overflow-y-auto min-h-0" enctype="multipart/form-data">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-1.5">
                        <label class="block text-xs font-semibold text-gray-700 uppercase tracking-wide">Bukti File <span class="text-red-500">*</span></label>
                        <div x-data="{ 
                            fileName: '', 
                            preview: null,
                            isPdf: false,
                        ...
                        <p class="text-[10px] text-gray-400 mt-1 uppercase tracking-wide">Format: JPG, PNG, PDF. Maks 5MB.</p>
                    </div>
                </div>

                <div class="space-y-1.5" x-show="modalType === 'in'">
                    <label class="block text-xs font-semibold text-gray-700 uppercase tracking-wide">Pengirim (Opsional)</label>
                    <input type="text" name="pengirim" placeholder="Nama / Vendor / Instansi pengirim"
                           class="block w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-yaksa-red/20 focus:border-yaksa-red shadow-sm transition-all text-gray-700 outline-none">
                </div>

                <div class="space-y-1.5" x-show="modalType === 'out'">
                    <label class="block text-xs font-semibold text-gray-700 uppercase tracking-wide">Penerima (Opsional)</label>
                ...
                <div class="pt-6 mt-6 border-t border-gray-100 flex justify-end gap-3">
                    <button type="button" @click="modalOpen = false" class="px-5 py-2.5 bg-white border border-gray-300 rounded-xl text-sm font-semibold text-gray-700 hover:bg-gray-50 transition-colors shadow-sm">Batal</button>
                    <button type="submit" class="px-5 py-2.5 text-white rounded-xl text-sm font-bold shadow-lg transition-all transform hover:-translate-y-0.5 focus:outline-none"
                            :class="modalType === 'in' ? 'bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 shadow-blue-500/30' : 'bg-gradient-to-r from-amber-600 to-amber-500 hover:from-amber-500 hover:to-amber-400 shadow-amber-500/30'">
                        Simpan Transaksi
                    </button>
                </div>
            </form>
...
<script>
    ...
    function confirmDeleteTx(txId) {
        Swal.fire({
            ...
            text: "Riwayat transaksi ini akan dihapus selamanya!",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#ef4444',
            cancelButtonColor: '#6b7280',
            ...
            if (result.isConfirmed) {
                document.getElementById('delete-tx-' + txId).submit();
            }
        })
    }
</script>
